...
import string
from evaluate_individu import Evaluate_ind

class Problem_Interface():
    def __init__(self, n_fields, gene_l ,gene_set):

        self.gene_l = gene_l
        self.genes = gene_set
        self.eval_inds = [Evaluate_ind(x) for x in n_fields]
    ...
    def evaluate_fitness(self, individu):
        ...
        """
        for x in self.eval_inds:
            x.set_path(individu.get_value()[0])
        scores_state = [x.complete_test() for x in self.eval_inds]
        scores = [x[0] for x in scores_state]
        individu.state_f = [x[1] for x in scores_state]
        f = sum(scores)
...
class Individual_Interface():
    def __init__(self,gene_l,gene_s,rand = True):
        ...
        self.__size = gene_l
        self.gene_s = gene_s
        self.state_f = [] # final state of the individu obtained using evaluate_ind

...
class Problem_Interface():
    def __init__(self,xmin,xmax,gene_set,gene_l):
        """
        :return: None
        :param xmin: the x min in the interval
        :type xmin: int
        :param xmax: the x max in the interval
        :type xmax: int
        :param gene_set: the set of genes used in that problem
        :type gene_set: List
        :param gene_l: the number of genes in the individu
        :type gene_l: int
    ...
    def evaluate_fitness(self, individu):
        ...
        """
        N = individu.get_value()[1]
        N_max = 2**self.gene_l - 1
        x = self.x_min + N*(self.x_max - self.x_min)/N_max
        f = (x**2)*sin(x)*cos(x)
    ...
    def compare_fitness(self,n1,n2):
        ...
        """
        # in that problem fitness should increase
...
class Individual_Interface():
    ...
    def get_value(self):
        ...
        return (self.__genome,int(self.__genome,2))
